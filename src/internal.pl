...
diagnose :-
    wc_format("Let's start with the diagnose", []),
...
ask_and_store_answer(Question) :-
    ...
    load_html_translations(Locale, Question, HtmlQuestion),
    append(HtmlQuestion, [ button([name(answer), type(submit), value("Yes"), accesskey("y")], YesLabel),
                                     button([name(answer), type(submit), value("No"), accesskey("n")], NoLabel)
                                     ], HtmlForm),
	wc_ask([ answer(Answer) ], HtmlForm),
